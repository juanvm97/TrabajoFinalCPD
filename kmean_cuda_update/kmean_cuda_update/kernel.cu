...
void createImage3(int* after_cluster, int K, int num_points, Point* points, int width, int height, Point* means)
{
	std::ofstream img("picture1.ppm");
	...
	for (int y = 0; y < width; y++) {
		for (int x = 0; x < height; x++) {
			...
			//std::cout << points[x * width + y].centroide << std::endl;
			int r = means[after_cluster[y * height + x]]._r;
			int g = means[after_cluster[y * height + x]]._g;
			int b = means[after_cluster[y * height + x]]._b;
...
void preprocessing_cuda(Point* mean, Point* mean_d, int* after_cluster_d, Point* points_d, int K, int num_points, Point* points, int* before_clusters, int* after_cluster, int blockX, int blockY, int x, int y)
{
	...
	Point* representative_points_d;
	CUDA_CALL(cudaMalloc((void**)&representative_points_d, sizeof(Point) * num_representative_points));

	CUDA_CALL(cudaMemcpy(representative_points_d, representative_points, sizeof(Point) * num_representative_points, cudaMemcpyHostToDevice));
	...
	for (int i = 0; i < 5; i++)
	{
		...
		CUDA_CALL(cudaMemcpy(after_cluster, after_cluster_d, sizeof(int) * num_representative_points, cudaMemcpyDeviceToHost));
...
int main(int argc, char* argv[])
{
	...
	preprocessing_cuda(mean, mean_d, after_cluster_d, points_d, K, num_points, points, before_clusters, after_cluster, blockX, blockY, x, y);
	...
	createImage3(after_cluster, K, num_points, points, x, y, mean);
